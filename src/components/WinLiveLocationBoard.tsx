...
const isValidPoint = (point?: WinLocationPoint | null) =>
...
export const WinLiveLocationBoard: React.FC<Props> = ({ pointA, pointB, pointC, className = '' }) => {
  const projected = useMemo(() => {
    ...
    const available = entries.filter(([, point]) => isValidPoint(point));
    if (!available.length) return [];

    const lats = available.map(([, point]) => point!.lat);
    const lngs = available.map(([, point]) => point!.lng);
  ...
  return (
    <div className={`relative h-[310px] overflow-hidden rounded-3xl border border-cyan-400/20 bg-[#071426] ${className}`}>
      ...
      <div className="absolute left-3 top-3 z-20 rounded-xl border border-cyan-400/20 bg-black/45 px-2.5 py-1.5 text-[9px] font-black text-cyan-200">
        <Crosshair className="mr-1 inline h-3.5 w-3.5" /> WIN LIVE LOCATION • NO MAP TILES
      </div>
      ...
      {projected.map(({ key, point, x, y, tone }) => {
        const toneClass = tone === 'cyan'
          ...
          : tone === 'emerald'
          ? 'border-emerald-300 bg-emerald-400 text-slate-950 shadow-emerald-400/40'
          : 'border-amber-300 bg-amber-400 text-slate-950 shadow-amber-400/40';
        return (
          <div key={key} className="absolute z-10 -translate-x-1/2 -translate-y-1/2" style={{ left: `${Math.max(9, Math.min(91, x))}%`, top: `${Math.max(13, Math.min(87, y))}%` }}>
      ...
      <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-[8px] text-slate-500">
        ...
        <span><MapPin className="mr-1 inline h-3 w-3" />พิกัดอ้างอิง ไม่ใช่เส้นทางถนน</span>
      </div>
    </div>
  );
};
