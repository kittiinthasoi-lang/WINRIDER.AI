          ...
          <div className="py-4 space-y-5">
            ...
            {activeTab === 'home' && (
              <div className="space-y-5">
                ...
                {/* Event Discovery Quick Anchor Strip */}
                <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
                  <button
                    type="button"
                    onClick={() => {
                      if (audioEnabled) playTactileBlip(800);
                      document.getElementById('daily-events-section')?.scrollIntoView({ behavior: 'smooth' });
                    }}
                    className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500/20 to-rose-500/20 border border-amber-400/50 text-[11px] font-bold text-amber-300 hover:border-amber-300 transition-all shadow-sm active:scale-95"
                  >
                    <span>🎉</span>
                    <span>อีเวนต์ & คอนเสิร์ตวันนี้</span>
                    <span className="px-1.5 py-0.2 rounded-full bg-amber-400 text-slate-950 text-[9px] font-black">ฮิต</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      if (audioEnabled) playTactileBlip(800);
                      setSelectedDestination('อิมแพ็ค อารีน่า เมืองทองธานี (IMPACT Arena)');
                      setShowBookingModal(true);
                    }}
                    className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-[#08152E] border border-white/10 text-[11px] font-medium text-slate-300 hover:text-white hover:border-white/20 transition-all active:scale-95"
                  >
                    <span>🎤</span>
                    <span>อิมแพ็ค อารีน่า</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      if (audioEnabled) playTactileBlip(800);
                      setSelectedDestination('ราชมังคลากีฬาสถาน / กกท. หัวหมาก');
                      setShowBookingModal(true);
                    }}
                    className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-[#08152E] border border-white/10 text-[11px] font-medium text-slate-300 hover:text-white hover:border-white/20 transition-all active:scale-95"
                  >
                    <span>⚽</span>
                    <span>ราชมังคลากีฬาสถาน</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      if (audioEnabled) playTactileBlip(800);
                      setSelectedDestination('ศูนย์การประชุมแห่งชาติสิริกิติ์ (QSNCC)');
                      setShowBookingModal(true);
                    }}
                    className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-[#08152E] border border-white/10 text-[11px] font-medium text-slate-300 hover:text-white hover:border-white/20 transition-all active:scale-95"
                  >
                    <span>📚</span>
                    <span>ศูนย์ฯ สิริกิติ์ QSNCC</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      if (audioEnabled) playTactileBlip(800);
                      setSelectedDestination('ตลาดนัดจ๊อดแฟร์ แดนเนรมิต');
                      setShowBookingModal(true);
                    }}
                    className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-[#08152E] border border-white/10 text-[11px] font-medium text-slate-300 hover:text-white hover:border-white/20 transition-all active:scale-95"
                  >
                    <span>🎪</span>
                    <span>จ๊อดแฟร์ แดนเนรมิต</span>
                  </button>
                </div>
