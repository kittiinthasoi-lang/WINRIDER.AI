...
export const WinAlertEventsCard: React.FC<WinAlertEventsCardProps> = ({
  ...
  const loadEvents = useCallback(async () => {
    ...
    setErrorMessage('');
    try {
      const originQuery = geo.isRealGps && Number.isFinite(geo.latitude) && Number.isFinite(geo.longitude) ? `&latitude=${encodeURIComponent(String(geo.latitude))}&longitude=${encodeURIComponent(String(geo.longitude))}` : '';
      const apiUrl = `/api/events/daily?date=${encodeURIComponent(today)}&country=TH${originQuery}`;
