...
export const WinAlertEventsCard: React.FC<WinAlertEventsCardProps> = ({
  ...
  const loadEvents = useCallback(async () => {
    ...
    setErrorMessage('');
    try {
      const apiUrl = `/api/events/daily?date=${encodeURIComponent(today)}&days=${rangeDays}&country=TH`;
  ...
  return (
    <section className={`space-y-3 ${className}`} aria-labelledby="daily-events-title">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-xs font-bold text-slate-400">ช่วงเวลา</span>
        {([1, 7, 30] as const).map((days) => (
          <button
            key={days}
            type="button"
            onClick={() => setRangeDays(days)}
            className={`rounded-xl border px-3 py-2 text-xs font-black ${rangeDays === days ? 'border-cyan-300 bg-cyan-300 text-slate-950' : 'border-white/10 bg-white/5 text-slate-300'}`}
          >
