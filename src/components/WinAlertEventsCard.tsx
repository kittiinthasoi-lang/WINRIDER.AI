...
export const WinAlertEventsCard: React.FC<WinAlertEventsCardProps> = ({
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || 'โหลดกิจกรรมจริงประจำวันไม่สำเร็จ');
    ...
    <section className={`space-y-3 ${className}`} aria-labelledby="daily-events-title">
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2"><h3 id="daily-events-title" className="text-base font-black text-white">กิจกรรมจริงประจำวันนี้</h3><span className="rounded-full border border-emerald-500/40 bg-emerald-500/15 px-2 py-0.5 text-xs font-bold text-emerald-300">ทั่วประเทศไทย</span></div>
            <p className="mt-1 text-sm text-slate-300">{formatDay(eventDate)}</p>
            <p className="mt-1 text-sm text-slate-400">งานลดราคา ตลาด/ป๊อปอัพ คอนเสิร์ต กีฬา เทศกาล และงานอีเวนต์ที่กำลังจัดจริงในวันนี้</p>
          </div>
        </div>
      ...
      {loading && <div className="flex min-h-36 items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/5 text-sm text-slate-300"><Loader2 className="h-5 w-5 animate-spin text-cyan-300" />กำลังตรวจสอบกิจกรรมจริงของวันนี้...</div>}
      {!loading && errorMessage && <div className="rounded-2xl border border-rose-400/30 bg-rose-500/10 p-4 text-sm text-rose-100"><div className="flex gap-2"><AlertCircle className="h-5 w-5 shrink-0" /><div><p className="font-bold">ยังโหลดกิจกรรมจริงไม่ได้</p><p className="mt-1 text-rose-200/80">{errorMessage}</p><p className="mt-2 text-xs text-slate-400">ระบบไม่แสดงกิจกรรมจำลองแทนข้อมูลจริง</p></div></div></div>}
      {!loading && !errorMessage && filteredEvents.length === 0 && <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-center"><CalendarDays className="mx-auto h-8 w-8 text-slate-500" /><p className="mt-3 font-bold text-white">ไม่พบกิจกรรมจริงในหมวดนี้สำหรับวันนี้</p><p className="mt-1 text-sm text-slate-400">งานที่จัดหลายวันจะแสดงทุกวันจนถึงวันสิ้นสุดโดยอัตโนมัติ</p></div>}
