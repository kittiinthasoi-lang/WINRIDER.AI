...
export const PublicDataDiscoveryCard:React.FC = () => {
  ...
  return <section className="rounded-3xl border border-cyan-400/15 bg-[#08152E] p-4 shadow-lg">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div><div className="flex items-center gap-2"><MapPin className="h-4 w-4 text-cyan-300"/><h2 className="text-sm font-black text-white">ข้อมูลสาธารณะประเทศไทย</h2><span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2 py-0.5 text-[8px] font-black text-emerald-300">FREE • SOURCE DIRECT</span></div><p className="mt-1 text-[9px] text-slate-500">ข้อมูลสาธารณะจากแหล่งต้นทาง • ใช้ได้กับทุกบทบาท • อัปเดตตามต้นทาง</p></div>
    ...
    <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
      {records.length===0&&!loading&&<div className="col-span-full rounded-2xl border border-dashed border-slate-800 p-5 text-center text-[10px] text-slate-500">ยังไม่มีข้อมูลที่ Admin Verify แล้วในหมวดนี้</div>}
      {records.map(item=><article key={item.id} className="rounded-2xl border border-slate-800 bg-slate-950/40 p-3">
        ...
        {Number.isFinite(Number(item.latitude))&&Number.isFinite(Number(item.longitude))&&<a href={'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent((item.name||item.title||'')+' '+(item.address||''))} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-[9px] font-bold text-cyan-300"><MapPin className="h-3 w-3"/>เปิดแผนที่</a>}
      </article>)}
    </div>
  </section>;
};
