...
export const ProfileQuickActions: React.FC<ProfileQuickActionsProps> = ({
  ...
  const open = async (tool: ProfileTool) => {
    ...
    try {
      ...
      setContactData({
        name: String(user.name || user.displayName || userName || ''),
        phone: String(user.phone || roleData.phone || ''),
        email: String(user.email || roleData.email || roleData.contactEmail || ''),
        emergencyName: String(emergency.name || ''),
        emergencyPhone: String(emergency.phone || ''),
        shopOrOrg: String(role === 'merchant' ? (roleData.shopName || roleData.name || '') : role === 'partner' ? (roleData.orgName || roleData.name || '') : ''),
        contactPerson: String(roleData.contactPerson || roleData.ownerName || ''),
        vehicle: String(role === 'knight' ? [roleData.vehicleType, roleData.plateNumber].filter(Boolean).join(' • ') : ''),
      });
    } catch (error) {
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2" aria-label="เครื่องมือโปรไฟล์">
      {actions.map(({ id, label, image }) => (
        ...
        >
          ...
          /> : <span className="flex h-20 w-full flex-col items-center justify-center gap-1 rounded-2xl border border-emerald-400/25 bg-emerald-400/10 text-emerald-300"><UserRound className="h-6 w-6" /><span className="text-[10px] font-black">ข้อมูลติดต่อจริง</span></span>}
        </button>
      ))}
    </div>
        ...
        {activeTool === 'contact' && (
          <div className="space-y-4">
            <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/5 p-4">
              <h3 className="text-sm font-black text-emerald-300">ข้อมูลที่ใช้ติดต่อสำหรับบทบาทนี้</h3>
              <p className="mt-1 text-xs text-slate-400">ดึงจากข้อมูลจริงที่กรอกตอนลงทะเบียนบัญชี ไม่ใช้ข้อมูลตัวอย่าง</p>
            </div>
            {contactLoading ? <div className="rounded-2xl border border-white/10 bg-white/5 p-8 text-center text-sm text-slate-400">กำลังโหลดข้อมูลจริง…</div> : contactData && (
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                {contactData.phone && <a href={`tel:${contactData.phone.replace(/\D/g, '')}`} className="rounded-2xl border border-emerald-400/20 bg-emerald-400/5 p-4"><Phone className="h-4 w-4 text-emerald-300" /><p className="mt-1 text-[10px] text-slate-500">โทรศัพท์</p><p className="font-black text-white">{contactData.phone}</p></a>}
