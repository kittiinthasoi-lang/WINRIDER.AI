...
export const WalletTopUpPanel: React.FC = () => {
  ...
  const [config, setConfig] = useState<{ configured: boolean; promptPayId: string; accountName: string } | null>(null);
  ...
  return <section className="rounded-3xl border border-emerald-400/40 bg-[#07172B] p-5 space-y-4">
    ...
    {!config?.configured ? <p className="rounded-xl bg-amber-500/10 p-3 text-xs text-amber-200">ยังไม่ได้ตั้งค่า ADMIN_PROMPTPAY_ID และ ADMIN_BANK_ACCOUNT_NAME</p> : <>
      <div className="grid gap-4 sm:grid-cols-[1fr_auto]">
        <div className="space-y-3"><label className="text-xs text-slate-300">ยอดที่ต้องการเติม (บาท)</label><input type="number" min="1" max="100000" value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="w-full rounded-xl border border-white/15 bg-black/30 p-3 text-white"/><p className="text-xs text-slate-300">ผู้รับ: <strong>{config.accountName}</strong><br/>PromptPay: {config.promptPayId}</p></div>
    ...
    <p className="text-[10px] text-slate-500"><QrCode className="mr-1 inline w-3 h-3"/>AI ช่วยคัดกรองเท่านั้น ไม่ได้ยืนยันว่าเงินเข้าธนาคาร ยอดจะเพิ่มหลัง Super Admin ตรวจและอนุมัติ</p>
  </section>;
};
